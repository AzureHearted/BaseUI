...
type BaseCustomDragOptions = {
	...
	onDragStart?: (startPos: Pos, event: PointerEvent, ghostDOM: HTMLElement | null) => void
	/** 拖拽移动的回调函数 (触发条件：拖拽状态下，鼠标指针移动) */
	onDragMove?: (pos: Pos, event: PointerEvent, ghostDOM: HTMLElement | null) => void
	/** 拖拽结束的回调函数 (触发条件：处于拖拽状态时候，鼠标松开; 提示：该函数支持异步) */
	onDrop?: (endPos: Pos, event: PointerEvent, ghostDOM: HTMLElement | null) => Promise<void> | void
}
 ...
 * @returns
 */
export function useCustomDrag(target: MaybeHTMLElementRef, options: BaseCustomDragOptions = {}) {
	// s 指针已按下
	...
	const onMove = (e: PointerEvent) => {
		...
		// 移动超过阈值才认为是拖拽
		if (pressed.value && !dragging.value && Math.abs(dx) + Math.abs(dy) > threshold) {
			dragging.value = true
		...
		if (dragging.value) {
			...
			if (options.ghostOffsetMethod) {
				...
				if (options.ghostOffsetMethod.x === 'auto') {
					...
					nowPos.x = e.clientX - targetStartPos.x - offsetPercent.x * ghostDOM.value.offsetWidth
				}
				// 垂直方向的定位方式
				if (options.ghostOffsetMethod.y === 'auto') {
					// 按照起始位置相对目标元素的百分比位置计算
					nowPos.y = e.clientY - targetStartPos.y - offsetPercent.y * ghostDOM.value.offsetHeight
				}
			}
