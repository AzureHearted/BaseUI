...
// ? imgWrap的Ref
const imgWrapRef = useTemplateRef(
  "imgWrapRef",
) as ShallowRef<HTMLDivElement | null>;

// s imgWarp尺寸数据
const imgWrapDimensions = ref({ width: 0, height: 0 });
...
// ? img标签的Ref
const imgRef = useTemplateRef("imgRef") as ShallowRef<HTMLImageElement | null>;
...
async function generateThumbnail<T = File | string>(
  ...
  return new Promise((resolve) => {
    ...
    img.onerror = function (error) {
      console.warn("[base-img] 缩略图生成出错", error, source);
      resolve("" as T);
    };
...
.base-img {
  ...
  // 加载时的样式
  &__loading[data-show-loading-animation="true"] &__wrapper {
    // 旋转动画
    @keyframes spin {
      0% {
    ...
    &::before {
      ...
      animation: spin 0.5s linear infinite; // 使用旋转动画
      z-index: 100;
    }
  }

  // 加载完成且可见的样式
  &__show &__wrapper > img {
    opacity: 1;
    visibility: visible;
    transition:
