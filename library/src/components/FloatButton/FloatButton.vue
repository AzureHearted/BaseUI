<template>
  <div ref="containerRef" class="base-float-button" :style="[style, sizeStyle]">
    ...
    <!-- 展开按钮容器 -->
    <div
      class="base-float-button__extra-wrapper"
      :style="{ '--total': extraVNode.length }"
    >
      <div
        v-for="(node, index) in extraVNode"
        :key="index"
        class="base-float-button__base-button base-float-button__extra-button"
        :style="{ '--i': index }"
      >
        <component :is="node" />
      </div>
...
const mainButtonRef = useTemplateRef(
  "mainButtonRef",
) as ShallowRef<HTMLDivElement | null>;
...
.base-float-button {
  ...
  &__extra-wrapper {
    ...
    /* 插槽内容尺寸 */
    & > .base-float-button__extra-button {
      @include base-button;
      --delay-step: 0.03s;
      width: var(--size);
      height: var(--size);
