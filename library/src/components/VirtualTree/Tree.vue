  ...
  >
    ...
    >
      <TreeNode
        ...
        :show-add-button="allowAddNode"
        :show-delete-button="allowDeleteNode"
      >
        <template #default>
...
// ? 自定义事件
const emits = defineEmits<VirtualTreeEmits>();
...
const state = reactive({
  data: [] as Array<TreeNodeItem>,
  // 展开节点的 Map
  expandNodeMap: new Map<string, TreeNodeItem>(),
  // 节点对象的 Map
  nodeMap: new Map<string, TreeNodeItem>(),
  // 组件冻结状态
  isFreeze: false,
});
...
interface FlattenResult {
  ...
  flat: TreeNodeItem[];
  /** 扁平化后的节点Map */
  nodeMap: Map<string, TreeNodeItem>;
}
...
function flattenAndBuildIndex(
  rootNodes: TreeNodeItem[] | null | undefined,
): FlattenResult {
  const flat: TreeNodeItem[] = [];
  ...
  if (props.hightlightFilterOnly) {
    for (const node of flat) {
      ...
      if (pNode.children?.length) {
        const stack: TreeNodeItem[] = [...pNode.children];
        while (stack.length) {
          const n = stack.pop()!;
...
// f 根据id查询节点 (遍历所有数去查找)
function getNodeById(id: string): TreeNodeItem | null {
 ...
 * @returns 子节点列表
 */
async function requestChildren(id: string): Promise<TreeNodeItem[]> {
  const node = getNodeById(id);
...
// 导出提供的provide类型
export type TreeContext = ReturnType<typeof createTreeContext>;
// 调用provide方法
provide(treeSymbol, createTreeContext());
